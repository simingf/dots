...
## Step 1 — Fetch local diff

Print `**Step 1 — Fetching local diff...**`

Read and execute the [fetch-local-diff](../fetch-local-diff/SKILL.md) skill in full. This produces `<REPO>`, `<BRANCH>`, and per-file diffs for each changed file. The fetch skill drops test files from the per-file diffs (unless the only changed files are tests, in which case they are kept).
